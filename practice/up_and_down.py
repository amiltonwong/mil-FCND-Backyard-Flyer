...
class Phases(Enum): # create a FSM (finite state machine)
    MANUAL = 0
    ARMING = 1
    TAKEOFF = 2
    LANDING = 3
...
class UpAndDownFlyer(Drone): # inherit from super class "Drone"

    def __init__(self, connection):
        super().__init__(connection) # call superclass's initializer
        self.target_position = np.array([0.0, 0.0, 0.0])
        self.in_mission = True # only begin to fly if declared in_mission = True 

        # initial state
        self.flight_phase = Phases.MANUAL

        # callbacks registration part:
        # register all your callbacks here
        # every time get message "position update" from autopilot, trigger callback funtion "local_position_callback"
    ...
    def local_position_callback(self):
        if self.flight_phase == Phases.TAKEOFF:
            ...
            if altitude > 0.95 * self.target_position[2]:
                self.landing_transition()  # call state transition function
